...
class Solution:
    def myAtoi(self, str):
        ...
        """
        result = 0
        vaild = {'+', '-', '1', '2', '3', '4', '5', '6', '7', '8', '9', '0'}
        vaild_num = {'1', '2', '3', '4', '5', '6', '7', '8', '9', '0'}
        int_maybe = str.lstrip(' ').split(' ')[0]

        if (len(int_maybe) > 0 and int_maybe[0] in vaild):
            int_str = int_maybe[0]
            for n in (int_maybe[1:]):
                if (n in vaild_num):
                    int_str = int_str + n
                else:
                    break
            if (len(int_str) == 1 and int_str in {'+', '-'}):
                result = 0
            else:
                result = int(int_str)
...
print(s.myAtoi('4193aace01 with words'))
