...
class Solution:
    def destCity(self, paths: List[List[str]]) -> str:
        ...
        for path in paths:
            ...
            distination.add(path[1])
        return (distination-departure).pop()


s = Solution()
print(s.destCity([["London", "New York"], [
      "New York", "Lima"], ["Lima", "Sao Paulo"]]))
print(s.destCity([["B", "C"], ["D", "B"], ["C", "A"]]))
print(s.destCity([["A", "Z"]]))
"""
此题解法：
题示中保证了路线不会循环，也就是说每个阶段的起点和终点最终可以连成一个单链。
a->b -> b->c -> c->d -> d->e ... 类似如此
可以使用集合来处理，也就是说把所有的起点都放在一个集合里，所有的终点都放在一个集合里
起点集合 与 终点集合 的差集就是初始的起点
终点集合 与 起点集合 的差集就是最终的终点
"""
