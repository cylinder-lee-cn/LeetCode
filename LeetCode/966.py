...
class Solution:
    def spellchecker(self, wordlist, queries):
        ...
        for idx, wl in enumerate(wordlist):
            if wd.get(wl, 0) == 0:
                wd[wl] = [idx]
            else:
                wd[wl].append(idx)
            ...
            wlupper = wl.upper()
            if wd.get(wlupper, 0) == 0:
                wd[wlupper] = [idx]
            else:
                wd[wlupper].append(idx)

            wlcut = cutvowel(wlupper)
            if wd.get(wlcut, 0) == 0:
                wd[wlcut] = [idx]
            else:
                wd[wlcut].append(idx)
...
"""
此题解法：
* 首先构造一个字典，存放原始单词（key）和对应索引（value），再增加转换成大写的单词，
  如果相同将索引append到value中，最后是将元音字母替换成‘0’的单词，
  如果相同将索引增加到value中
* 遍历queries，先查找原始单词是否在字典中，如果有返回最小索引，并跳到下一个循环
  如果没有，就查有没有全部大写的，如果有就返回最小索引，并跳到下一循环
  如果没有，就查替换掉元音字母的单词，如果有就返回最小索引，如果没有就返回‘’
"""
